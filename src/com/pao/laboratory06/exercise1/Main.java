...
public class Main {
    public static void main(String[] args) {
        ...
        Scanner scanner = new Scanner(System.in);
        int numarAngajati = scanner.nextInt(); // 1
        Angajat[] angajati = new Angajat[numarAngajati]; // 2
        for (int i = 0; i < numarAngajati; i++) {
            angajati[i] = Angajat.citeste(scanner);
        }
        String criterion = scanner.next(); // 3

        Comparator<Angajat> comparator = switch (criterion) {
            case "by_salary" -> (Angajat a1, Angajat a2) -> Double.compare(a1.getSalariu(), a2.getSalariu()); // part A
            case "by_name" -> (Angajat a1, Angajat a2) -> a1.getNume().compareTo(a2.getNume()); // part B
            case "by_salary_desc" -> (Angajat a1, Angajat a2) -> Double.compare(a2.getSalariu(), a1.getSalariu()); // part C
            default -> (a1, a2) -> 0; // nu sortam
        };
